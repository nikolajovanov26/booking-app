            ...
            <table class="text-left w-full">
                <div class="p-4 flex items-center justify-end bg-blue-grad-dark">
                    <div class="flex items-center">
                        <a href="{{ route('dashboard.rooms.create', ['property' => $property]) }}"
                    ...
                    @foreach($rooms as $room)
                        <tr class="hover:bg-gray-100 transition">
                            ...
                            <td class="flex items-center justify-end px-6 py-4 text-right space-x-3">
                                <a href="{{ route('dashboard.rooms.edit', ['room' => $room->id, 'property' => $property]) }}"
                                   class="bg-blue-600 text-white rounded px-3 py-2 hover:bg-blue-800 transition">Edit</a>
                                <form method="post"
                                      action="{{ route('dashboard.rooms.destroy', ['property' => $property->id, 'room' => $room->id]) }}">
                                    @csrf
                                    @method('DELETE')
